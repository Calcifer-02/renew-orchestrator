...
class CleanupSmokeBody(BaseModel):
    prefix: str
...
# ─── Token timeline & audit ───────────────────────────────────────────────────

@app.get("/api/tokens/{task_id}/timeline")
def api_token_timeline(task_id: str) -> dict:
    """Full history for a specific token."""
    return get_token_timeline(_dirs(), task_id)


@app.get("/api/audit/consistency")
def api_audit_consistency() -> dict:
    """Consistency check: replay events vs actual marking."""
    return run_replay_bridge(_dirs(), dry_run=True)


@app.get("/api/audit/replay")
def api_audit_replay() -> dict:
    """Replay dry-run summary (alias for consistency)."""
    return run_replay_bridge(_dirs(), dry_run=True)


@app.post("/api/debug/cleanup-smoke")
def api_cleanup_smoke(body: CleanupSmokeBody) -> dict:
    """Remove smoke test tokens by prefix. DEBUG endpoint — prefix ≥ 2 chars required."""
    try:
        return cleanup_smoke_tokens(_dirs(), body.prefix)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc))
