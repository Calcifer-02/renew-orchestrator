import type { NetDefinition, OrchestratorState, Task, Job, TickResult, TokenTimeline, AuditStatus } from '../types'
...
export async function fetchTimeline(task_id: string): Promise<TokenTimeline> {
  const res = await fetch(`${BASE}/api/tokens/${encodeURIComponent(task_id)}/timeline`)
  if (!res.ok) await _throw(res)
  return res.json()
}

export async function fetchConsistency(): Promise<AuditStatus> {
  const res = await fetch(`${BASE}/api/audit/consistency`)
  if (!res.ok) await _throw(res)
  return res.json()
}
...
export async function cleanupSmoke(
  prefix: string,
): Promise<{ removed_tasks: string[]; removed_jobs: string[]; prefix: string }> {
  const res = await fetch(`${BASE}/api/debug/cleanup-smoke`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
