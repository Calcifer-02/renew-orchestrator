...
export function PlaceNode({ data }: NodeProps) {
  const { label, tokens, color, hasTokens, selectedTaskId, onTokenClick } =
    data as unknown as PlaceNodeData
      ...
      <div style={{
        ...
        {tokens.length > 0 && (
          <div style={{
            ...
            {tokens.map((tid) => {
              const selected = tid === selectedTaskId
              return (
                <div
                  key={tid}
                  title={tid}
                  onClick={(e) => { e.stopPropagation(); onTokenClick?.(tid) }}
                  style={{
                    width: selected ? 12 : 9,
                    ...
                    boxShadow: selected
                      ? `0 0 10px #ffffff, 0 0 20px ${color}`
                      : undefined,
          ...
          <div style={{
            ...
            {tokens.map((tid) => (
              <span
                ...
                style={{
                  cursor: onTokenClick ? 'pointer' : 'default',
                  color: tid === selectedTaskId ? '#ffffff' : `${color}dd`,
                  fontWeight: tid === selectedTaskId ? 700 : 400,
                }}
              >
                {tid}{' '}
              </span>
        ...
        <div style={{
          ...
          textAlign: 'center', lineHeight: 1.25, letterSpacing: '0.6px',
          whiteSpace: 'pre-line',
