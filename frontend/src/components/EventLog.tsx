  ...
  return (
    <section className="flex flex-col flex-1 min-h-0">
      <header className="px-4 py-2 flex items-center justify-between shrink-0 border-b border-[#0e1e35]">
        ...
        <span className="text-[9px] font-mono text-[#1e3a5f]">{events.length} событий</span>
      </header>
...
function EventRow({ ev }: { ev: BridgeEvent }) {
  ...
  if (ev.type === 'transition_fired' && ev.transition)
    ...
    detail = ev.success ? '✓ успешно' : '✗ ошибка'
  else if (ev.type === 'bridge_started')
    detail = 'runtime запущен'

  return (
    <div
      className="px-4 py-1.5 border-b border-[#060e1a] hover:bg-[#050d1a] transition-colors group"
