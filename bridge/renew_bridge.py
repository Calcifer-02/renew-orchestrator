...
def replay(dirs: ProjectDirs, dry_run: bool = True) -> dict:
    """
    Enhanced event audit / replay tool.

    Reads events.ndjson, reconstructs expected marking from transition_fired events,
    compares against actual marking from refactor-plan.json, and checks job/budget
    event consistency.  Returns structured warnings and reconstructed_marking.
    """
    if not dirs.events_file.exists():
        return {
            "ok": True, "warnings": [], "reconstructed_marking": {},
            "actual_marking": {}, "total": 0, "by_type": {}, "dry_run": dry_run,
        }
    ...
    reconstructed:   dict[str, str] = {}   # task_id → last known place
    job_created_ids: set[str]        = set()
    job_closed_ids:  set[str]        = set()
    ...
    for i, line in enumerate(lines, 1):
        ...
        try:
            ...
            warnings.append({"line": i, "issue": "invalid JSON"})
            continue
        ...
        if not ev.get("event_id"):
            warnings.append({"line": i, "event_type": ev_type, "issue": "missing event_id"})

        if ev_type == "task_created":
            task_id = ev.get("task_id")
            if task_id:
                reconstructed[task_id] = ev.get("to_place", "BACKLOG")

        elif ev_type == "transition_fired":
            task_id  = ev.get("task_id")
            to_place = ev.get("to_place")
            if task_id and to_place:
                if task_id not in reconstructed:
                    warnings.append({
                        "line": i, "event_type": ev_type,
                        "issue": f"transition_fired for unknown task {task_id!r}",
                    })
                reconstructed[task_id] = to_place

        elif ev_type == "job_created":
            job_id = ev.get("job_id")
            if job_id:
                job_created_ids.add(job_id)

        elif ev_type in ("job_completed", "job_failed"):
            job_id = ev.get("job_id")
            if job_id:
                if job_id not in job_created_ids:
                    warnings.append({
                        "line": i, "event_type": ev_type,
                        "issue": f"job closed without job_created: {job_id[:8]}",
                    })
                job_closed_ids.add(job_id)

        elif ev_type == "budget_spent":
            job_id = ev.get("job_id", "")
            if job_id and job_id not in job_closed_ids:
                warnings.append({
                    "line": i, "event_type": ev_type,
                    "issue": f"budget_spent without job_completed: job {job_id[:8]}",
                })

        elif ev_type == "smoke_cleanup":
            for tid in ev.get("removed_task_ids", []):
                reconstructed.pop(tid, None)

    # Compare reconstructed vs actual marking
    actual_tasks = _load_tasks(dirs)
    actual: dict[str, str] = {t["task_id"]: t["place"] for t in actual_tasks}

    for task_id, expected_place in reconstructed.items():
        actual_place = actual.get(task_id)
        if actual_place is not None and actual_place != expected_place:
            warnings.append({
                "task_id": task_id,
                "issue":   (f"place mismatch: reconstructed={expected_place!r}, "
                            f"actual={actual_place!r}"),
            })
    ...
    for task_id in actual:
        if task_id not in reconstructed:
            warnings.append({
                "task_id": task_id,
                "issue":   "task in actual marking has no task_created event",
            })

    for job in _load_jobs(dirs):
        if job["status"] in ("pending", "running") and job["job_id"] not in job_created_ids:
            warnings.append({
                "job_id": job["job_id"][:8],
                "issue":  "active job without job_created event",
            })
    ...
    return {
        "ok":                    len(warnings) == 0,
        "warnings":              warnings,
        "reconstructed_marking": reconstructed,
        "actual_marking":        actual,
        "total":                 total,
        "by_type":               by_type,
        "dry_run":               dry_run,
    }
...
# ─── Token timeline & audit helpers ──────────────────────────────────────────

def _event_to_timeline_entry(ev: dict) -> dict | None:
    """Convert a raw event dict to a human-readable timeline entry."""
    ev_type = ev.get("type", "")
    ts      = ev.get("ts", "")
    task_id = ev.get("task_id")

    if ev_type == "task_created":
        return {"ts": ts, "type": ev_type, "task_id": task_id,
                "summary": f"Токен создан в {ev.get('to_place', 'BACKLOG')}"}

    if ev_type == "transition_fired":
        tr  = ev.get("transition", "?")
        frm = ev.get("from_place", "?")
        to  = ev.get("to_place",   "?")
        return {"ts": ts, "type": ev_type, "task_id": task_id,
                "transition": tr, "from_place": frm, "to_place": to,
                "summary": f"Переход {tr}: {frm} → {to}"}

    if ev_type == "job_created":
        role = ev.get("agent_role", "?")
        return {"ts": ts, "type": ev_type, "task_id": task_id,
                "job_id": ev.get("job_id", ""), "agent_role": role,
                "summary": f"Создано задание {role}"}

    if ev_type == "job_completed":
        role = ev.get("agent_role", "?")
        return {"ts": ts, "type": ev_type, "task_id": task_id,
                "job_id": ev.get("job_id", ""), "agent_role": role,
                "summary": f"Задание {role} завершено"}

    if ev_type == "job_failed":
        role   = ev.get("agent_role", "?")
        reason = ev.get("reason", "")
        return {"ts": ts, "type": ev_type, "task_id": task_id,
                "job_id": ev.get("job_id", ""), "agent_role": role, "reason": reason,
                "summary": f"Задание {role} провалено"}

    if ev_type == "resource_locks_released":
        return None  # skip noise

    if ev_type == "smoke_cleanup":
        return {"ts": ts, "type": ev_type, "task_id": task_id,
                "summary": "Токен удалён (smoke cleanup)"}

    return {"ts": ts, "type": ev_type, "task_id": task_id,
            "summary": ev_type.replace("_", " ")}


def get_token_timeline(dirs: ProjectDirs, task_id: str) -> dict:
    """
    Build full history for a specific token.
    Sources: events.ndjson, agent_jobs.json, economic_decisions.ndjson.
    """
    tasks   = _load_tasks(dirs)
    task    = next((t for t in tasks if t["task_id"] == task_id), None)
    jobs    = [j for j in _load_jobs(dirs) if j["task_id"] == task_id]
    job_ids = {j["job_id"] for j in jobs}

    timeline_events: list[dict] = []

    if dirs.events_file.exists():
        for line in dirs.events_file.read_text(encoding="utf-8").strip().splitlines():
            if not line.strip():
                continue
            try:
                ev = json.loads(line)
            ...
            if ev.get("task_id") != task_id:
                continue
            entry = _event_to_timeline_entry(ev)
            if entry:
                timeline_events.append(entry)

    econ_path     = dirs.reports / "economic_decisions.ndjson"
    budget_events: list[dict] = []
    if econ_path.exists():
        for line in econ_path.read_text(encoding="utf-8").strip().splitlines():
            if not line.strip():
                continue
            try:
                ev = json.loads(line)
                ...
                continue
            if ev.get("task_id") != task_id and ev.get("job_id") not in job_ids:
                continue
            budget_events.append(ev)
            dt     = ev.get("decision_type", "")
            ts     = ev.get("ts", "")
            amount = ev.get("amount")
            if dt == "budget_spent":
                timeline_events.append({
                    "ts": ts, "type": "budget_spent", "task_id": task_id,
                    "amount": amount, "summary": f"Бюджет списан: {amount}",
                })
            elif dt == "candidate_budget_blocked":
                timeline_events.append({
                    "ts": ts, "type": "budget_blocked", "task_id": task_id,
                    "summary": "Бюджет заблокировал действие",
                })
    ...
    return {
        "task_id":       task_id,
        "current_place": task["place"] if task else None,
        "task":          task,
...
def task_report(dirs: ProjectDirs, task_id: str) -> str:
    """Generate a markdown-like text report for a task."""
    tl   = get_token_timeline(dirs, task_id)
    task = tl.get("task")

    if task is None:
        return f"# Task Report: {task_id}\n\nTask not found.\n"

    human_tr = {"approve_plan", "escalate_review", "escalate", "reopen", "close_wontfix"}

    out = [
        f"# Task Report: {task_id}",
        "",
        f"Current place: {task.get('place', '?')}",
        f"Status:        {task.get('status', '?')}",
        f"Attempts:      {task.get('attempt', 0)}",
    ...
    for ev in tl["events"]:
        ts      = (ev.get("ts") or "")[:19]
        summary = ev.get("summary") or ev.get("type", "?")
        out.append(f"- [{ts}] {summary}")

    out.extend(["", "## Jobs"])
    for job in tl["jobs"]:
        out.append(
            f"- {job.get('agent_role','?')}: {job.get('status','?')} ({job.get('job_id','?')[:8]})"
        )
    if not tl["jobs"]:
        out.append("- (нет заданий)")

    fired = [ev for ev in tl["events"] if ev.get("type") == "transition_fired"]
    if fired:
        out.extend(["", "## Transitions"])
        for tr in fired:
            out.append(
                f"- {tr.get('from_place','?')} → {tr.get('to_place','?')} via {tr.get('transition','?')}"
            )

    human = [ev for ev in tl["events"]
             if ev.get("type") == "transition_fired"
             and ev.get("transition") in human_tr]
    if human:
        out.extend(["", "## Human decisions"])
        for hd in human:
            out.append(f"- {hd.get('transition','?')}")

    budget_events = tl.get("budget_events", [])
    if budget_events:
        out.extend(["", "## Economy"])
        spent = sum(ev.get("amount", 0) for ev in budget_events
                    if ev.get("decision_type") == "budget_spent")
        out.append(f"- Total spent: {spent:.2f}")
        for ev in budget_events[-5:]:
            dt     = ev.get("decision_type", "?")
            ts     = (ev.get("ts") or "")[:19]
            amount = ev.get("amount")
            if amount is not None:
                out.append(f"- [{ts}] {dt}: {amount}")
            else:
                out.append(f"- [{ts}] {dt}")

    out.extend(["", "## Consistency"])
    rp         = replay(dirs, dry_run=True)
    task_warns = [w for w in rp.get("warnings", []) if w.get("task_id") == task_id]
    if task_warns:
        for w in task_warns:
            out.append(f"- WARNING: {w.get('issue', str(w))}")
    else:
        out.append("- OK")
...
def cleanup_smoke_tokens(dirs: ProjectDirs, prefix: str) -> dict:
    """
    Remove smoke test tokens whose task_id starts with prefix.
    Writes a smoke_cleanup event but does NOT delete events.ndjson.
    prefix must be >= 2 chars and not '*'.
    """
    if not prefix or len(prefix) < 2 or prefix == "*":
        raise ValueError(
            f"Invalid prefix {prefix!r} — must be ≥ 2 chars and not '*'."
        )
    ...
    smoke_tasks   = [t for t in tasks if t["task_id"].startswith(prefix)]
    if not smoke_tasks:
        return {"removed_tasks": [], "removed_jobs": [], "prefix": prefix}

    smoke_ids     = {t["task_id"] for t in smoke_tasks}
    smoke_jobs    = [j for j in jobs  if j["task_id"] in smoke_ids]
    smoke_job_ids = {j["job_id"] for j in smoke_jobs}

    _save_tasks(dirs, [t for t in tasks if t["task_id"] not in smoke_ids])
    _save_jobs(dirs,  [j for j in jobs  if j["task_id"] not in smoke_ids])
    _save_locks(dirs, [l for l in locks if l.get("job_id") not in smoke_job_ids])
    _update_marking_md(dirs, [t for t in tasks if t["task_id"] not in smoke_ids])

    _emit(dirs, {
        "type":             "smoke_cleanup",
        "prefix":           prefix,
        "removed_task_ids": sorted(smoke_ids),
        "removed_job_ids":  sorted(smoke_job_ids),
        "source":           "bridge",
    })

    return {
        "removed_tasks": sorted(smoke_ids),
        "removed_jobs":  sorted(smoke_job_ids),
        "prefix":        prefix,
    }
...
def _cli() -> None:
    ...
    p_replay = sub.add_parser("replay", help="Audit events.ndjson (consistency check)")
    p_replay.add_argument("--dry-run", action="store_true", default=True)

    p_report = sub.add_parser("report", help="Print markdown report for a task")
    p_report.add_argument("task_id")

    p_cleanup = sub.add_parser("cleanup-smoke", help="Remove smoke tokens by prefix")
    p_cleanup.add_argument("--prefix", required=True, metavar="PREFIX")
    ...
    try:
        if args.cmd == "marking":
            ...
            print(task_report(dirs, args.task_id))

        elif args.cmd == "cleanup-smoke":
            print(json.dumps(cleanup_smoke_tokens(dirs, args.prefix), indent=2))
